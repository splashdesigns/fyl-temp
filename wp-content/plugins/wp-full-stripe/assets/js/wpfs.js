...
(function ($) {
    ...
    $(function () {
            ...
            function resetCaptcha(formHash) {
            ...
            function removeTokenValueInput($form) {
                $form.find('input[name="wpfs-stripe-token"]').remove();
            }
            ...
            function addTokenValueInput($form, token) {
                if (typeof(token) !== 'undefined' && token.hasOwnProperty('id')) {
                    $('<input>').attr({
                        type: 'hidden',
                        name: 'wpfs-stripe-token',
                        value: token.id
                    }).appendTo($form);
                }
            }
             ...
             * @param token
             */
            function stripeTokenHandler($form, card, token) {
                clearGlobalMessage($form);
                clearFieldErrors($form);
                addTokenValueInput($form, token);
             ...
             * @returns {{open, close}|*}
             */
            function createCheckoutHandler($form) {
                return StripeCheckout.configure({
                    ...
                    token: function (token, args) {
                        ...
                        // tnagy remove Stripe token and email and billing address fields from previous submit, JIRA reference WPFS-458
                        $('input[name="wpfs-stripe-token"]', $form).remove();
                        $('input[name="wpfs-card-holder-email"]', $form).remove();
                        $('input[name="wpfs-billing-name"]', $form).remove();
                        $('input[name="wpfs-billing-address-country"]', $form).remove();
                        $('input[name="wpfs-billing-address-zip"]', $form).remove();
                        $('input[name="wpfs-billing-address-state"]', $form).remove();
                        $('input[name="wpfs-billing-address-line-1"]', $form).remove();
                        $('input[name="wpfs-billing-address-line-2"]', $form).remove();
                        $('input[name="wpfs-billing-address-city"]', $form).remove();
                        $('input[name="wpfs-shipping-name"]', $form).remove();
                        $('input[name="wpfs-shipping-address-country"]', $form).remove();
                        $('input[name="wpfs-shipping-address-zip"]', $form).remove();
                        $('input[name="wpfs-shipping-address-state"]', $form).remove();
                        $('input[name="wpfs-shipping-address-line-1"]', $form).remove();
                        $('input[name="wpfs-shipping-address-line-2"]', $form).remove();
                        $('input[name="wpfs-shipping-address-city"]', $form).remove();

                        var showBillingAddress = $form.data('wpfs-show-billing-address');
                        var showShippingAddress = $form.data('wpfs-show-shipping-address');
                        ...
                        $('<input>').attr({
                            type: 'hidden',
                            name: 'wpfs-card-holder-email',
                            value: token.email
                        }).appendTo($form);

                        if (showBillingAddress == 1) {
                            var $billingName = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-billing-name',
                                value: args.billing_name
                            });
                            var $billingAddressCountry = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-billing-address-country',
                                value: args.billing_address_country_code
                            });
                            var $billingAddressCountryCode = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-billing-address-country-code',
                                value: args.billing_address_country_code
                            });
                            var $billingAddressCountryName = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-billing-address-country-name',
                                value: args.billing_address_country
                            });
                            var $billingAddressZip = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-billing-address-zip',
                                value: args.billing_address_zip
                            });
                            var $billingAddressState = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-billing-address-state',
                                value: args.billing_address_state
                            });
                            var $billingAddressLine1 = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-billing-address-line-1',
                                value: args.billing_address_line1
                            });
                            var $billingAddressCity = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-billing-address-city',
                                value: args.billing_address_city
                            });
                            $form.append($billingName, $billingAddressCountry, $billingAddressCountryCode, $billingAddressCountryName, $billingAddressZip, $billingAddressState, $billingAddressLine1, $billingAddressCity);
                        }
                        if (showShippingAddress == 1) {
                            var $shippingName = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-shipping-name',
                                value: args.shipping_name
                            });
                            var $shippingAddressCountry = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-shipping-address-country',
                                value: args.shipping_address_country_code
                            });
                            var $shippingAddressCountryCode = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-shipping-address-country-code',
                                value: args.shipping_address_country_code
                            });
                            var $shippingAddressCountryName = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-shipping-address-country-name',
                                value: args.shipping_address_country
                            });
                            var $shippingAddressZip = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-shipping-address-zip',
                                value: args.shipping_address_zip
                            });
                            var $shippingAddressState = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-shipping-address-state',
                                value: args.shipping_address_state
                            });
                            var $shippingAddressLine1 = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-shipping-address-line-1',
                                value: args.shipping_address_line1
                            });
                            var $shippingAddressCity = $('<input>').attr({
                                type: 'hidden',
                                name: 'wpfs-shipping-address-city',
                                value: args.shipping_address_city
                            });
                            $form.append($shippingName, $shippingAddressCountry, $shippingAddressCountryCode, $shippingAddressCountryName, $shippingAddressZip, $shippingAddressState, $shippingAddressLine1, $shippingAddressCity);
                        }

                        $form.data('wpfs-closed-by', 'token_callback');
                        ...
                        $.ajax({
                            ...
                            error: function () {
                                showErrorGlobalMessage($form, wpfsL10n.internal_error_title, wpfsL10n.internal_error);
                            },
                            complete: function () {
                                enableFormButtons($form);
                                hideLoadingAnimation($form);
                    ...
                    closed: function () {
                        var closedBy = $form.data('wpfs-closed-by');
            ...
            WPFS.initStripeJSCard = function () {
                ...
                $cards.each(function (index) {
                    ...
                    card.mount('div.wpfs-form-control[data-wpfs-form-id="' + formId + '"]');
                    card.addEventListener('change', function (event) {
                        if (event.error) {
                            clearFieldError($form, 'cardnumber', null);
                            showFieldError($form, 'cardnumber', null, event.error.message);
                    ...
                    $form.submit(function (event) {
                        ...
                        // tnagy create and validate Stripe token
                        var tokenData = {};
                        ...
                        if (billingAddressCountryField.length > 0) {
                            ...
                            if (billingAddressCountry != null && billingAddressCountry != '') {
                                tokenData.address_country = billingAddressCountry;
                            }
                        }
                        ...
                        stripe.createToken(card, tokenData).then(function (result) {
                            ...
                            if (result.error) {
                                enableFormButtons($form);
                                hideLoadingAnimation($form);
                                showFieldError($form, 'cardnumber', null, result.error.message);
                                scrollToElement($('.wpfs-form-card', $form), false);
                            } else {
                                try {
                                    stripeTokenHandler($form, card, result.token);
                                } catch (e) {
                                    logException(formId, e);
                                    showErrorGlobalMessage($form, wpfsL10n.stripe_errors.internal_error_title, e.message);
                                    enableFormButtons($form);
                                    hideLoadingAnimation($form);
                                }
                            }
            ...
            WPFS.initPopupForms = function () {
                ...
                $popupForms.each(function (index, popupForm) {
                    ...
                    $form.submit(function (event) {
                        ...
                        var companyName = $form.data('wpfs-company-name');
                        var productDesc = $form.data('wpfs-product-description');
                        var buttonTitle = $form.data('wpfs-button-title');
                        var showBillingAddress = $form.data('wpfs-show-billing-address');
                        var showShippingAddress = $form.data('wpfs-show-shipping-address');
                        var showRememberMe = $form.data('wpfs-show-remember-me');
                        var image = $form.data('wpfs-image');
                        var useAlipay = $form.data('wpfs-use-alipay');
                        var preferredLanguage = $form.data('wpfs-preferred-language');

                        var handler = createCheckoutHandler($form);
                        var billingAddress = (showBillingAddress == 1);
                        var shippingAddress = (showShippingAddress == 1);
                        if (shippingAddress) {
                            billingAddress = true;
                        }
                        ...
                        if (shippingAddress) {
                            options.shippingAddress = shippingAddress;
                        }
            ...
            WPFS.initReCaptcha = function () {
                window.addEventListener('load', function () {
                    ...
                    var formHash;
                    var inlineFormCAPTCHAs = document.getElementsByClassName('wpfs-inline-form-captcha');
                    //noinspection JSUnresolvedVariable
                    if (window.grecaptcha !== 'undefined' && inlineFormCAPTCHAs !== null && inlineFormCAPTCHAs.length > 0) {
                        //noinspection JSUnresolvedVariable
                        googleReCAPTCHA = window.grecaptcha;
                        Array.prototype.forEach.call(inlineFormCAPTCHAs, function (element) {
