			...
			<div class="" id="wpfs-subscription-forms">
				...
				<div style="min-height: 200px;">
					...
					<?php if ( count( $checkout_subscription_forms ) === 0 ): ?>
						<p class="alert alert-info">
							<?php esc_html_e( "No popup subscription forms created yet. Use the 'Create Popup Form' button to get started.", 'wp-full-stripe' ); ?>
						</p>
					<?php else: ?>
						<table class="wp-list-table widefat fixed subscription-forms">
							<thead>
							...
							<?php foreach ( $checkout_subscription_forms as $checkout_subscription_form ): ?>
								...
								<tr>
									...
									<td class="column-vat-rate">
										<?php
										if ( MM_WPFS::VAT_RATE_TYPE_NO_VAT == $checkout_subscription_form->vatRateType ) {
											$vatRateLabel = __( 'No VAT', 'wp-full-stripe' );
										} elseif ( MM_WPFS::VAT_RATE_TYPE_FIXED_VAT == $checkout_subscription_form->vatRateType ) {
											$vatRateLabel = sprintf( '%s%%', round( $checkout_subscription_form->vatPercent, 4 ) );
										} elseif ( MM_WPFS::VAT_RATE_TYPE_CUSTOM_VAT == $checkout_subscription_form->vatRateType ) {
											$vatRateLabel = __( 'Custom VAT', 'wp-full-stripe' );
										} else {
											$vatRateLabel = __( 'Unknown VAT rate Type', 'wp-full-stripe' );
										}
